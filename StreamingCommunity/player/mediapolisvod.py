...
class VideoSource:
    ...
    def extract_m3u8_url(video_url: str) -> str:
        ...
        try:
            ...
            video_obj = video_data.get("video") if isinstance(video_data, dict) else None
            if not video_obj:
                return "Error: 'video' key missing in response"
            ...
            try:
                ...
                video_list = stream_data.get("video", []) if isinstance(stream_data, dict) else []
                m3u8_url = video_list[0] if isinstance(video_list, list) and video_list else None
            except Exception:
                try:
