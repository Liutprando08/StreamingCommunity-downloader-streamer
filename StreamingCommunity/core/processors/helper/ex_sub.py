...
def extract_font_name_from_style(style_line: str) -> str | None:
    ...
    try:
        ...
        console.print(
            f"[red]Error extracting font name from line: {style_line.strip()}: {e}"
        )
        return None
...
def process_subtitle_fonts(subtitle_path: str):
    ...
    if not installed_fonts:
        console.print(
            "[red]Error: No fonts detected on system. Cannot process subtitle fonts."
        )
        return
    ...
    try:
        ...
        console.print(f"[red]Error reading subtitle file {subtitle_path}: {e}")
        return
    ...
    for i, line in enumerate(lines):
        if line.startswith("Style:"):
            ...
            if font_name is None:
                console.print(
                    f"[yellow]Warning: Could not parse Style line {i + 1}: {line.strip()}"
                )
                continue
    ...
    if missing_fonts:
        for font in sorted(missing_fonts):
            console.print(
                f"[yellow][{system}] No font found for '{font}' in {os.path.basename(subtitle_path)}"
            )

    if not found_fonts and not missing_fonts:
        console.print(
            f"[yellow]No Style definitions found in {os.path.basename(subtitle_path)}"
        )
...
def detect_subtitle_format(subtitle_path: str) -> str | None:
    ...
    try:
        ...
        with open(subtitle_path, "r", encoding="utf-8", errors="ignore") as f:
            ...
            if (
                re.search(r"\[script info\]", first_lines, re.IGNORECASE)
                or re.search(r"\[v4\+ styles\]", first_lines, re.IGNORECASE)
                or re.search(r"\[v4 styles\]", first_lines, re.IGNORECASE)
            ):
                if re.search(
                    r"format:\s*name", first_lines, re.IGNORECASE
                ) or re.search(r"format:\s*marked", first_lines, re.IGNORECASE):
                    return "ass"
                return "ssa"
        ...
        console.print(f"[red]Error detecting subtitle format for {subtitle_path}: {e}")
...
def fix_subtitle_extension(subtitle_path: str) -> str:
    ...
    if detected_format is None:
        console.print(
            f"[yellow]    Warning: Could not detect format for {subtitle_path}, keeping original extension"
        )
    ...
    try:
        ...
        console.print(
            f"[yellow]    Renamed subtitle: [cyan]{current_ext} [yellow]-> [cyan]{detected_format}"
        )
