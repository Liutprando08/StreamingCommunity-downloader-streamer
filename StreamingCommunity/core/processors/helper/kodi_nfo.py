...
def _build_movie_nfo(details: dict, output_dir: str, poster_url: Optional[str] = None, fanart_url: Optional[str] = None) -> str:
...
def _build_episode_nfo(details: dict, season_num: Optional[int] = None, episode_num: Optional[int] = None) -> str:
...
class KodiNFOGenerator:
    ...
    def _resolve_tmdb(self, slug: str, media_type: str):
        ...
        tmdb_type = 'movie' if media_type and media_type.lower() in ('film', 'movie') else 'tv'
        result = tmdb.get_type_and_id_by_slug_year(slug, media_type=tmdb_type)
        if result and result.get('id'):
            return result
        return None

    def generate_for_movie(self, slug: Optional[str] = None):
        ...
        if not tmdb_info or not tmdb_info.get('id'):
            logging.warning(f"[KodiNFO] Could not resolve TMDB ID for movie: {slug}")
            return False

        details = tmdb.get_full_details('movie', tmdb_info['id'])
        if not details:
            logging.warning(f"[KodiNFO] Could not fetch details for TMDB ID {tmdb_info['id']}")
            return False
        ...
        if images.get('posters'):
            ...
            if tmdb.download_image(images['posters'][0], poster_path):
                ...
                logging.info(f"[KodiNFO] Downloaded poster for {slug}")
        ...
        if images.get('backdrops'):
            ...
            if tmdb.download_image(images['backdrops'][0], fanart_path):
                ...
                logging.info(f"[KodiNFO] Downloaded fanart for {slug}")

        nfo_content = _build_movie_nfo(details, str(self.output_dir), poster_url, fanart_url)
        nfo_path = os.path.join(self.output_dir, 'movie.nfo')
        with open(nfo_path, 'w', encoding='utf-8') as f:
            f.write(nfo_content)
        logging.info(f"[KodiNFO] Created movie.nfo for {slug}")
        return True

    def generate_for_tvshow(self, slug: Optional[str] = None):
        ...
        if not tmdb_info or not tmdb_info.get('id'):
            logging.warning(f"[KodiNFO] Could not resolve TMDB ID for show: {slug}")
            return False
        ...
        with open(nfo_path, 'w', encoding='utf-8') as f:
            f.write(nfo_content)
        logging.info(f"[KodiNFO] Created tvshow.nfo for {slug}")
        return True

    def generate_for_episode(self, slug: Optional[str] = None, season_num: Optional[int] = None, episode_num: Optional[int] = None):
        ...
        tmdb_info = self._resolve_tmdb(slug, 'tv')
        if not tmdb_info or not tmdb_info.get('id'):
            logging.warning(f"[KodiNFO] Could not resolve TMDB ID for episode: {slug}")
            return False
        ...
        with open(nfo_path, 'w', encoding='utf-8') as f:
            f.write(nfo_content)
        logging.info(f"[KodiNFO] Created episode NFO for S{season_num}E{episode_num} of {slug}")
        return True


def generate_kodi_metadata(file_path: str, media_type: Optional[str] = None):
    """
    ...
    if not api_key:
        logging.warning("[KodiNFO] TMDB API key not configured. Skipping metadata generation.")
        return
