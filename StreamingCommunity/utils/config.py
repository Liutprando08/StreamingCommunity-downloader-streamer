...
class ConfigAccessor:
    ...
    def get(
        self, section: str, key: str, data_type: type = str, default: Any = None
    ) -> Any:
        """
        ...
        if section not in self._config_dict:
            if default is not None:
                return default
            raise ValueError(
                f"Section '{section}' not found in {self._cache_prefix} configuration"
            )
        ...
        if key not in self._config_dict[section]:
            if default is not None:
                return default
            raise ValueError(
                f"Key '{key}' not found in section '{section}' of {self._cache_prefix} configuration"
            )
    ...
    def _convert_to_data_type(self, value: Any, data_type: type) -> Any:
        ...
        try:
            ...
            raise ValueError(
                f"Error converting: {data_type.__name__} to value '{value}' with error: {e}"
            )

    def get_int(self, section: str, key: str, default: int | None = None) -> int:
        """Read an integer from the configuration."""
        return self.get(section, key, int, default=default)

    def get_float(self, section: str, key: str, default: float | None = None) -> float:
        """Read a float from the configuration."""
        return self.get(section, key, float, default=default)

    def get_bool(self, section: str, key: str, default: bool | None = None) -> bool:
        """Read a boolean from the configuration."""
        return self.get(section, key, bool, default=default)

    def get_list(
        self, section: str, key: str, default: list[str] | None = None
    ) -> list[str]:
        """Read a list from the configuration."""
        return self.get(section, key, list, default=default)

    def get_dict(self, section: str, key: str, default: dict | None = None) -> dict:
...
class ConfigManager:
    def __init__(self) -> None:
        ...
        self.config = ConfigAccessor(
            self._config_data, self.cache, "config", self._cache_enabled
        )
        self.login = ConfigAccessor(
            self._login_data, self.cache, "login", self._cache_enabled
        )
        self.domain = ConfigAccessor(
            self._domains_data, self.cache, "domain", self._cache_enabled
        )
        self.remote_cdm = ConfigAccessor(
            self._remote_cdm_data, self.cache, "remote_cdm", self._cache_enabled
        )
    ...
    def _load_config(self) -> None:
        ...
        if not os.path.exists(self.config_file_path):
            console.print(
                f"[red]WARNING: Configuration file not found: {self.config_file_path}"
            )
            console.print("[yellow]Downloading from repository...")
            self._download_file(
                CONFIG_DOWNLOAD_URL, self.config_file_path, "config.json"
            )
        ...
        try:
            ...
            console.print(f"[red]Error parsing config JSON: {e!s}")
            self._handle_config_error()

        except OSError as e:
            console.print(f"[red]Error loading configuration: {e!s}")
            self._handle_config_error()
    ...
    def _load_login(self) -> None:
        ...
        if not os.path.exists(self.login_file_path):
            console.print(
                f"[yellow]WARNING: Login file not found: {self.login_file_path}"
            )
            console.print("[yellow]Downloading from repository...")
            try:
                self._download_file(
        ...
        try:
            ...
            console.print(f"[red]Error parsing login JSON: {e!s}")
            self._login_data.clear()

        except OSError as e:
            console.print(f"[red]Error loading login configuration: {e!s}")
            self._login_data.clear()
    ...
    def _load_remote_cdm(self) -> None:
        ...
        if not os.path.exists(self.remote_cdm_path):
            console.print(
                f"[yellow]WARNING: Remote cdm file not found: {self.remote_cdm_path}"
            )
            console.print("[yellow]Downloading from repository...")
            try:
                self._download_file(
        ...
        try:
            ...
            console.print(f"[red]Error parsing remote cdm JSON: {e!s}")
            self._remote_cdm_data.clear()

        except OSError as e:
            console.print(f"[red]Error loading remote cdm configuration: {e!s}")
            self._remote_cdm_data.clear()
        ...
        for section, key, data_type in common_keys:
            try:
                ...
                if section in self._config_data and key in self._config_data[section]:
                    ...
                    converted_value = self.config._convert_to_data_type(
                        value, data_type
                    )
    ...
    def _handle_config_error(self) -> None:
        ...
        try:
            ...
            console.print(f"[red]Critical configuration error: {e!s}")
    ...
    def _download_file(self, url: str, file_path: str, file_name: str) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                console.print(
                    f"[green]Download complete: {file_name} ({file_size:.2f} KB)"
                )
            else:
            ...
            console.print(f"[red]Download error: {e!s} for url: {url}")
            raise

    def _load_site_data(self) -> None:
        """Load site data from local file first, then online fallback."""
        self._load_site_data_from_file()
        if not self._domains_data and self.fetch_domain_online:
    ...
    def _load_site_data_online(self) -> None:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                console.print(
                    f"[red]GitHub request failed: HTTP {response.status_code}, {response.text[:100]}"
                )
    ...
    def _save_domains_to_appropriate_location(self) -> None:
        ...
        try:
            ...
            console.print(f"[red]Could not save domains to file: {save_error!s}")
    ...
    def _load_site_data_from_file(self) -> None:
        ...
        try:
            if os.path.exists(self.domains_path):
                ...
                site_count = (
                    len(self._domains_data)
                    if isinstance(self._domains_data, dict)
                    else 0
                )

            elif os.path.exists(self.github_domains_path):
                console.print(
                    f"[cyan]Fallback domain path: [green]{self.github_domains_path}"
                )
                ...
                console.print(
                    f"[green]Domains loaded from GitHub structure: {site_count} streaming services"
                )
            ...
            console.print(f"[red]Local domain file error: {e!s}")
            self._domains_data.clear()
    ...
    def _handle_site_data_fallback(self) -> None:
        ...
        if os.path.exists(self.domains_path):
            ...
            try:
                ...
                console.print(f"[red]Conf domains fallback failed: {fallback_error!s}")

        if os.path.exists(self.github_domains_path):
            console.print(
                "[yellow]Attempting fallback to GitHub structure domains.json file..."
            )
            try:
                with open(self.github_domains_path, "r", encoding="utf-8") as f:
                    self._domains_data.clear()
                ...
                console.print(
                    f"[red]GitHub structure fallback failed: {fallback_error!s}"
                )
