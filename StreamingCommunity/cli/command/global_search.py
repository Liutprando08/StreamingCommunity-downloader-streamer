...
def global_search(search_terms: str | None = None, selected_sites: list | None = None):
    ...
    if selected_sites is None:
        ...
        choice = msg.ask(
            "[green]Enter your choice (1-3)", choices=["1", "2", "3"], default="1"
        )
        ...
        if choice == "1":
            ...
            category_choices = msg.ask(
                "[green]Enter category numbers separated by commas", default="1"
            )
            selected_categories = [
                list(sites_by_category.keys())[int(c.strip()) - 1]
            ...
            site_choices = msg.ask(
                "[green]Enter site numbers separated by commas", default="1"
            )
            selected_indices = [int(c.strip()) - 1 for c in site_choices.split(",")]
            selected_sites = [
                list(search_functions.keys())[i]
                for i in selected_indices
                if i < len(search_functions)
            ]
    ...
    for alias in selected_sites:
        ...
        try:
            ...
            if (
                database
                and hasattr(database, "media_list")
                and len(database.media_list) > 0
            ):
            ...
            console.print(f"[red]Error searching {site_name}: {e!s}")

    # Display the consolidated results
    if all_results:
        all_media_items = [item for results in all_results.values() for item in results]
...
def process_selected_item(selected_item, search_functions):
    ...
    if not source_alias or source_alias not in search_functions:
        console.print(
            "[red]Error: Cannot process this item - source information missing."
        )
        return
    ...
    if item_id is not None:
        console.print(
            f"[green]Selected item: {item_title} (ID: {item_id}, Type: {item_type})"
        )
        ...
        try:
            ...
            console.print(f"[red]Error processing download: {e!s}")
