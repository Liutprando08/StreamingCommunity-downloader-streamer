...
def _cfg_dict(section, key, default=None):
    return config_manager.config.get_dict(section, key, default=default if default is not None else {})
...
class MediaDownloader:
    ...
    def _match_external_subtitle_lang(self, ext_lang: str) -> bool:
        ...
        try:
            ...
            return any(t.lower() in ext_lang.lower() for t in re.findall(r"[A-Za-z]{2,}", sub_filter))
        except Exception:
            return False
    ...
    def parser_stream(self) -> List[StreamInfo]:
        ...
        norm_v = self._normalize_filter(filters['video'] if filters and filters.get('video') else _cfg("DOWNLOAD", "select_video"))
        norm_a = self._normalize_filter(filters['audio'] if filters and filters.get('audio') else _cfg("DOWNLOAD", "select_audio"))
        norm_s = self._normalize_filter(filters['subtitle'] if filters and filters.get('subtitle') else _cfg("DOWNLOAD", "select_subtitle"))
    ...
    def start_download(self) -> Dict[str, Any]:
        ...
        # Determine filters
        norm_v = self._normalize_filter(filters['video'] if filters and 'video' in filters else ("best" if getattr(self, "force_best_video", False) else _cfg("DOWNLOAD", "select_video")))
        norm_a = self._normalize_filter(filters['audio'] if filters and 'audio' in filters else _cfg("DOWNLOAD", "select_audio"))
        norm_s = self._normalize_filter(filters['subtitle'] if filters and 'subtitle' in filters else _cfg("DOWNLOAD", "select_subtitle"))
