...
class SegmentDownloader:
    ...
    def _save_segment(self, segment, output_dir, encryption_method=None, key_data=None, iv=None, decryptor=None):
        """Salva un segmento su disco, applicando la decrittazione se necessaria."""
    ...
    def download_all(self, segments, output_dir, description="segments", stream_type="media", language="und", resolution="", encryption_method=None, key_data=None, iv=None, decryptor=None):
        ...
        def update_progress(pct, speed_str="", size_str="", segments_str=""):
        ...
        with DownloadBarManager(self.download_id) as bar_mgr:
                ...
                for future in as_completed(futures):
                    ...
                    try:
                        ...
                        if success:
                            ...
                            self._save_segment(segment, output_dir, encryption_method, key_data, iv, decryptor)
                            ...
                            failed_count += 1
                            update_progress((downloaded_count + failed_count) / total_segments * 100 if total_segments > 0 else 0)
            ...
            # Secondo passaggio: retry seriale dei segmenti ancora falliti
            if failed_count > 0:
                console.print(f"[yellow]Retrying {failed_count} failed segment(s) serially...")
                for seg in segments:
                    if self.is_cancelled():
                        break
                    if seg.downloaded:
                        continue
                    with failed_segments_lock:
                        failed_segments.discard(seg.number)
                    if self.download_segment(seg):
                        downloaded_count += 1
                        failed_count -= 1
                        total_size += seg.size
                        self._save_segment(seg, output_dir, encryption_method, key_data, iv, decryptor)
                        progress_percent = (downloaded_count / total_segments * 100) if total_segments > 0 else 0
                        update_progress(progress_percent, segments_str=f"{downloaded_count}/{total_segments}")
