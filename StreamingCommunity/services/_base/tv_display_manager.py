...
def dynamic_format_number(number_str: str) -> str:
    ...
    try:
        ...
        logger.warning(
            f"Could not format episode number '{number_str}': {e!s}. Using original format."
        )
        return number_str


def manage_selection(cmd_insert: str, max_count: int | None) -> list[int]:
    """
    ...
    while True:
        ...
        try:
            ...
            if list_selection:
                list_selection = sorted(set(list_selection))
                break
...
def map_episode_title(
    tv_name: str, number_season: int, episode_number: int, episode_name: str
) -> str:
    """
    ...
    if tv_name is not None:
        map_episode_temp = map_episode_temp.replace(
            "%(tv_name)", os_manager.get_sanitize_file(tv_name)
        )
    ...
    if number_season is not None:
        ...
        map_episode_temp = map_episode_temp.replace(
            "%(season)", dynamic_format_number(str(0))
        )

    if episode_number is not None:
        map_episode_temp = map_episode_temp.replace(
            "%(episode)", dynamic_format_number(str(episode_number))
        )
    else:
        map_episode_temp = map_episode_temp.replace(
            "%(episode)", dynamic_format_number(str(0))
        )

    if episode_name is not None:
        map_episode_temp = map_episode_temp.replace(
            "%(episode_name)", os_manager.get_sanitize_file(episode_name)
        )
...
def validate_selection(
    list_season_select: list[int], available_seasons: list[int]
) -> list[int]:
    """
    ...
    while True:
        try:
            ...
            valid_seasons = [
                season for season in list_season_select if season in available_seasons
            ]

            # If the list is empty, the input was completely invalid
            if not valid_seasons:
                input_seasons = msg.ask(
                    f"[red]Enter valid season numbers ({', '.join(map(str, available_seasons))})"
                )
            ...
            logger.error("Error: Please enter valid integers separated by commas.")

            # Prompt the user for valid input again
            input_seasons = input(
                f"Enter valid season numbers ({', '.join(map(str, available_seasons))}): "
            )
            list_season_select = list(map(int, input_seasons.split(",")))
...
def display_seasons_list(seasons_manager) -> str:
    ...
    try:
        has_type = (
            hasattr(seasons_manager.seasons[0], "type")
            and (seasons_manager.seasons[0].type) is not None
            and str(seasons_manager.seasons[0].type) != ""
        )
        has_id = (
            hasattr(seasons_manager.seasons[0], "id")
            and (seasons_manager.seasons[0].id) is not None
            and str(seasons_manager.seasons[0].id) != ""
        )
    except IndexError:
        has_type = False
        has_id = False
...
def display_episodes_list(episodes_manager) -> str:
    ...
    for media in episodes_manager:
        category = (
            ...
            else getattr(media, "category", None)
        )
        duration = (
            media.get("duration")
            if isinstance(media, dict)
            else getattr(media, "duration", None)
        )
