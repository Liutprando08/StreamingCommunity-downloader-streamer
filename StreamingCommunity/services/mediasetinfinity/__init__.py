...
def title_search(query: str) -> int:
    ...
    # Safe nested access to avoid IndexError/AttributeError on chained .get()[0]
    items = []
    try:
        data = resp_json.get("data", {})
        search_page = data.get("getSearchPage", {})
        containers_conn = search_page.get("areaContainersConnection", {})
        containers = containers_conn.get("areaContainers", [])
        if containers:
            areas = containers[0].get("areas", [])
            if areas:
                sections = areas[0].get("sections", [])
                if sections:
                    collections = sections[0].get("collections", [])
                    if collections:
                        items_conn = collections[0].get("itemsConnection", {})
                        items = items_conn.get("items", [])
