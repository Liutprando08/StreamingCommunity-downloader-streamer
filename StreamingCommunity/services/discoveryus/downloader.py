...
def download_episode(
    ...
    if (
        str(playback_info["type"]).strip().lower() != "dash"
        or playback_info["license_url"] is None
    ):
        console.print(
            f"[red]Unsupported streaming type. Playback info: {playback_info}"
        )
        return None, False

    if playback_info["license_token"] is None:
        console.print("[red]No license token available for this content.")
        return None, False
...
def download_series(
    ...
    start_message()
    if not scrape_serie:
        id_parts = str(select_season.id).split("|")
